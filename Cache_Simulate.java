...
class Cache {
    ...
    private void updateCache(int setIndex, int blockIndex, Block fetchedBlock, int address) {
        ...
        if (debugMode2) {
            System.out.println("Updating cache with setIndex of " + setIndex + ", blockIndex of " + blockIndex
             + ", and address of " + address);
        }
        setsArr[setIndex].blocksArr[blockIndex] = fetchedBlock;
        setsArr[setIndex].blocksArr[blockIndex].setIsMappedToAddress(true);
        int blockDistance = address % blockSizeInWords;
        int blockAddress = address - blockDistance;
        setsArr[setIndex].blocksArr[blockIndex].setMappedAddress(blockAddress);
        accessBlock(setIndex, blockIndex);
        if (debugMode2) {
            System.out.println("blockDistance: " + blockDistance);
            System.out.println("blockAddress: " + blockAddress);
            System.out.println("is mapped address after updating: " + setsArr[setIndex].blocksArr[blockIndex].getIsMappedToAddress());
            System.out.println("mapped address after updating: " + setsArr[setIndex].blocksArr[blockIndex].getMappedAddress());
        }
    }
...
class Set {
    ...
    public int findBlockIndexInSet(int address3, Cache cache) {
        for (int i = 0; i < blocksArr.length; i++) {
            int blockDistance = address3 % cache.getBlockSizeInWords();
            int blockAddress = address3 - blockDistance;
            if (blocksArr[i].getIsMappedToAddress() && blocksArr[i].getMappedAddress() == blockAddress) { // hit
        ...
        for (int i = 0; i < blocksArr.length; i++) {
            if (blocksArr[i] == leastRecentlyUsedBlock) {
                ...
                if (leastRecentlyUsedBlock.getIsDirty()) {
                    ...
                    if (leastRecentlyUsedBlock.getMappedAddress() != -1)
                        cache.logCacheAction(leastRecentlyUsedBlock.getMappedAddress(), cache.getBlockSizeInWords(), Cache.ActionType.CACHE_TO_NOWHERE);
                }
